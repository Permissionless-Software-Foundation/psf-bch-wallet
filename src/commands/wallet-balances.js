...
class WalletBalances extends Command {
    ...
    async getBalances(filename) {
        try {
            ...
            const bchBalance = this.bchWallet.bchjs.BitcoinCash.toBitcoinCash(satBalance);
    ...
    displayBalance(walletData, flags = {}) {
        try {
            ...
            console.log(`BCH balance: ${walletData.satBalance} satoshis or ${walletData.bchBalance} BCH`);
            // console.log(
            ...
            const mintBatons = walletData.utxos.utxoStore.slpUtxos.type1.mintBatons.concat(walletData.utxos.utxoStore.slpUtxos.group.mintBatons);
            ...
            if (flags.verbose) {
                console.log(`\nUTXO information:\n${JSON.stringify(walletData.utxos.utxoStore, null, 2)}`);
            }
            return true;
        }
        catch (err) {
